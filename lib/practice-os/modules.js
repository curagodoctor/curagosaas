...
export function resolveMissionModules(mission) {
  ...
  if (list.length) {
    return list
      .map((m, i) => ({
        ...
        aiPrompt: m.aiPrompt || '',
        // Multi-prompt: prefer the array; fall back to the single legacy prompt.
        aiPrompts: (Array.isArray(m.aiPrompts) && m.aiPrompts.length)
          ? m.aiPrompts.filter((p) => String(p || '').trim())
          : (m.aiPrompt ? [m.aiPrompt] : []),
  ...
  return [{
    ...
    // The mission's stored prompt (e.g. the daily-engine Excel task) is exposed
    // as the fireable prompt so the assistant AUTO-RUNS it and shows the output
    // (the "chatbot runs the pre-existing prompt" behaviour).
    aiPrompt: mission?.aiContext?.systemPrompt || '',
    aiPrompts: mission?.aiContext?.systemPrompt ? [mission.aiContext.systemPrompt] : [],
    aiSystemPrompt: '',
