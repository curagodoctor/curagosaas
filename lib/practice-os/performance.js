...
export async function getOrCreatePerformance(doctorId) {
  let p = await PerformanceScore.findOne({ doctorId });
  if (!p) p = await PerformanceScore.create({ doctorId });
  return p;
}

// A completed mission. onTime = finished within 24h of becoming available.
export async function recordCompletion(doctorId, { onTime = true, hasEvidence = false, hasReflection = false } = {}) {
  const p = await getOrCreatePerformance(doctorId);
  if (onTime) {
    p.executionScore += 10;
  } else {
...
export async function recordSkip(doctorId) {
  const p = await getOrCreatePerformance(doctorId);
...
// +2 once per day for showing up.
export async function recordLogin(doctorId) {
  const p = await getOrCreatePerformance(doctorId);
...
// +2 once per day for using the assistant effectively.
export async function recordAiUse(doctorId) {
  const p = await getOrCreatePerformance(doctorId);
  const today = startOfDay();
