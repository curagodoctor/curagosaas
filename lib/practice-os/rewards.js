...
export async function awardAchievement(doctorId, data) {
  ...
  // Mirror every achievement onto the journey timeline (same pack).
...
export async function onMissionCompleted(doctorId, mission, { streak = 0 } = {}) {
  ...
  if (streak > 0 && streak % 7 === 0) {
    const streakAch = { type: 'streak', title: `${streak}-mission streak`, message: `You've kept going ${streak} missions in a row.`, xp: 0, frameworkId };
  ...
  if (mission.weekNumber) {
    const weekMissions = await Mission.find({ frameworkId, weekNumber: mission.weekNumber, status: 'published' }).select('_id').lean();
    if (weekMissions.length > 0) {
      ...
      if (doneCount >= weekMissions.length) {
        const weekAch = { type: 'weekly', title: `Week ${mission.weekNumber} complete`, message: `You finished every mission in week ${mission.weekNumber}.`, xp: 0, frameworkId };
  ...
  const total = await Mission.countDocuments({ frameworkId, status: 'published' });
  const done = await UserMissionProgress.countDocuments({ doctorId, frameworkId, status: { $in: ['completed', 'skipped'] } });
  if (total > 0 && done >= total) {
    const fwAch = { type: 'framework', title: 'Programme complete', message: 'You built a real digital presence, one mission at a time.', xp: 0, frameworkId };
    await awardAchievement(doctorId, fwAch);
    await addJourney(doctorId, { type: 'milestone', title: 'Practice established', description: 'Completed the full programme.', frameworkId });
