...
export async function hasPackAccess(doctorId, frameworkOrId) {
  ...
  // §3 v1 — no payment wall: optimization packs unlock via the founder's Get
  // Access grant (optimizationAccess), not (only) a Razorpay purchase.
  if ((fw.tier || 'optimization') === 'optimization' && await hasOptimizationAccess(doctorId)) return true;
...
export async function hasOptimizationAccess(doctorId) {
  ...
  return !!profile?.optimizationAccess?.granted;
}
