...
export async function structureLongContent({ instruction, source = '', profileFields = {}, frameworkId = null, topic = '' }) {
  ...
  try {
    ...
    const completion = await client.chat.completions.create({
      ...
      response_format: { type: 'json_object' },
      messages: [
        { role: 'system', content: 'You write comprehensive, publish-ready long-form content for an Indian doctor building their online presence. Follow NMC norms strictly: informative and educational, never advertising, no superlatives, no comparative or guaranteed-outcome claims, no soliciting patients. Ground everything in the doctor profile + knowledge base provided; never invent credentials, statistics, prices, or clinic details that are not given. Return ONLY a single JSON object matching the requested shape.' },
