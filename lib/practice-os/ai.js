...
function buildSystemPrompt(authoredPrompt, mission, profileContext) {
  const context = [
  ...
  const authored = (authoredPrompt || '').trim();
 ...
 */
export async function runMissionAssistant({ mission, userPrompt, profileContext, history = [] }) {
  ...
  try {
    ...
    const system = buildSystemPrompt(mission?.aiContext?.systemPrompt, mission, profileContext);
