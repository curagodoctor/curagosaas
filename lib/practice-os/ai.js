...
/**
 * Fill {{placeholder}} tokens with the doctor's real profile values, server-side.
 * The AUTHOR writes a generic prompt with placeholders ("a {{specialty}} in
 * {{city}}"); the doctor never sees the filled version, only the assistant's
 * output. Unknown/empty tokens collapse to nothing so no "{{...}}" ever leaks.
 */
...
function buildSystemPrompt(authoredPrompt, mission, profileContext, profileFields = {}) {
  const context = [
 ...
 */
export async function runMissionAssistant({ mission, userPrompt, profileContext, profileFields = {}, history = [] }) {
  ...
  try {
    ...
    const system = buildSystemPrompt(mission?.aiContext?.systemPrompt, mission, profileContext, profileFields);
