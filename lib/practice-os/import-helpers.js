...
// Classify a task's PRIMARY button (from the Excel Primary_Button_* columns) into
// one of the three rotating action types. The Excel author writes the intent in
// Primary_Button_Action (e.g. "blog", "gbp", "gemini") — or the text/link hints.
export function classifyPrimaryAction({ text = '', action = '', link = '' } = {}) {
  const hay = `${action} ${text} ${link}`.toLowerCase();
  if (/gemini/.test(hay)) {
    return { type: 'gemini', label: String(text).trim() || 'Open Gemini GBP chat', url: String(link).trim() || '{{gemini_gbp_chat_link}}' };
  }
  if (/gbp|google\s*business|business\s*profile|\bgmb\b/.test(hay)) {
    return { type: 'gbp', label: String(text).trim() || 'Open Google Business Profile', url: String(link).trim() || '{{gbp_link}}' };
  }
  if (/blog|push/.test(hay) || (!action && !link)) {
    return { type: 'blog', label: String(text).trim() || 'Push as blog page', url: '' };
  }
  // Anything else with a link → a plain external button.
  return { type: 'custom', label: String(text).trim() || 'Open', url: String(link).trim() };
}
