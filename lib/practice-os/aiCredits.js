...
// blog/page generate). Paid packs grant an accumulating daily pool (default
// 30/day); the free tier gets a small one-time lifetime pool (§4). The ledger
// picks the right pool based on the doctor's paid status.
import AiCreditLedger from '@/models/practice-os/AiCreditLedger';
import { hasAiAccess } from '@/lib/practice-os/access';

// A large sentinel returned as "remaining" for unlimited (admin-granted) doctors.
export const UNLIMITED_CREDITS = 999999;

export async function getRemainingCredits(doctorId) {
  const paid = await hasAiAccess(doctorId);
  const ledger = await AiCreditLedger.getOrCreateForToday(doctorId, paid);
  if (ledger.unlimited) return UNLIMITED_CREDITS;
...
export async function chargeAiCredits(doctorId, { amount = 1, label = '', tokens = 0 } = {}) {
  const paid = await hasAiAccess(doctorId);
  const ledger = await AiCreditLedger.getOrCreateForToday(doctorId, paid);
  // Unlimited (admin-granted): log usage/tokens but never decrement.
  if (ledger.unlimited) {
...
export async function assertHasCredits(doctorId, amount = 1) {
  const remaining = await getRemainingCredits(doctorId);
  if (remaining < Math.max(1, amount)) {
