import Doctor from '@/models/Doctor';
import Clinic from '@/models/Clinic';
import PracticeOsProfile from '@/models/practice-os/PracticeOsProfile';

/**
 * The doctor-global Practice OS profile (CV knowledge base + intent). Created on
 * first access. Setup is entered once and shared across every pack the doctor owns.
 */
export async function getOrCreateProfile(doctorId) {
  let p = await PracticeOsProfile.findOne({ doctorId });
  if (!p) p = await PracticeOsProfile.create({ doctorId });
  return p;
}

// Mirror the doctor's APPROVED disease clusters into their profile so `diseases`
// and `procedures` are the single source of truth (used by content generation,
// the website, GBP posts, etc.). Called whenever a cluster is approved.
export async function syncClustersToProfile(doctorId) {
  const PracticeOsDiseaseCluster = (await import('@/models/practice-os/PracticeOsDiseaseCluster')).default;
  const approved = await PracticeOsDiseaseCluster.find({ doctorId, approved: true }).sort({ order: 1 }).lean();
  if (!approved.length) return;
  const diseases = approved.map((c) => c.name).filter(Boolean).join(', ');
  const procedures = [...new Set(approved.flatMap((c) => (c.treatments || []).map((t) => t.name)).filter(Boolean))].join(', ');
  const profile = await getOrCreateProfile(doctorId);
  profile.credentials = profile.credentials || {};
  const kept = Array.isArray(profile.credentials.extracted)
    ? profile.credentials.extracted.filter((e) => e.field !== 'diseases' && e.field !== 'procedures')
    : [];
  kept.push({ field: 'diseases', value: diseases, confidence: 1, confirmed: true });
  if (procedures) kept.push({ field: 'procedures', value: procedures, confidence: 1, confirmed: true });
  profile.credentials.extracted = kept;
  profile.markModified('credentials');
  await profile.save();
}

/**
 * Practice OS — doctor knowledge base.
 *
 * Builds the profile context injected into the mission assistant so it drafts
 * with the doctor's REAL details (confirmed CV fields + a CV excerpt), never
 * generic filler. Returns '' when there's nothing yet.
 */
const FIELD_LABELS = {
  doctor_name: 'Name',
  designation: 'Designation',
  specialty: 'Specialty',
  subspecialty: 'Subspecialty',
  qualifications: 'Qualifications',
  additional_qualifications: 'Fellowships / additional qualifications',
  years_experience: 'Years of experience',
  expertise: 'Areas of expertise',
  diseases: 'Common diseases treated',
  procedures: 'Procedures / treatments',
  usp: 'Unique strength',
  interests: 'Areas of interest',
  languages: 'Languages',
  awards: 'Awards',
  publications: 'Publications',
  registration: 'Registration number',
  age: 'Age',
  gender: 'Gender',
  clinic_name: 'Clinic name',
  clinic_address: 'Clinic address',
  city: 'City',
  state: 'State',
  pin_code: 'PIN code',
  consultation_timings: 'Consultation timings',
  consultation_fee: 'Consultation fee',
  appointment_number: 'Appointment number',
  whatsapp_number: 'WhatsApp number',
  writing_style: 'Preferred writing style',
  doctor_personality: 'Personality (how patients should describe them)',
  custom_instructions: 'Custom instructions for the assistant',
};

/**
 * Generate a short professional summary of the doctor from their profile, for
 * display in the UI. Factual, third-person, NMC-compliant. Returns '' if no
 * profile yet or AI unconfigured.
 */
export async function generateDoctorSummary(doctorId) {
  if (!process.env.OPENAI_API_KEY) return '';
  const ctx = await getDoctorProfileContext(doctorId);
  if (!ctx) return '';
  try {
    const OpenAI = (await import('openai')).default;
    const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
    const completion = await client.chat.completions.create({
      model: process.env.OPENAI_MODEL || 'gpt-4o',
      max_tokens: 300,
      messages: [
        { role: 'system', content: 'Write a warm, factual 2–3 sentence professional summary of this doctor, in third person, for their own CuraGo dashboard. Use ONLY the facts provided — never invent. Keep it NMC-compliant: no superlatives, no comparative or guaranteed-outcome claims.' },
        { role: 'user', content: ctx },
      ],
    });
    return (completion.choices?.[0]?.message?.content || '').trim();
  } catch (e) {
    console.error('[Practice OS summary]', e.message);
    return '';
  }
}

/**
 * The doctor's profile as a flat { field: value } map — used to fill the
 * {{placeholder}} tokens in the (visible) per-mission prompt, server-side.
 */
export async function getDoctorProfileFields(doctorId) {
  const [doctor, profile, primaryClinic] = await Promise.all([
    Doctor.findById(doctorId).select('name displayName specialization clinicName whatsappNumber phone subdomain customDomain customDomainVerified').lean(),
    PracticeOsProfile.findOne({ doctorId }).select('credentials variables').lean(),
    Clinic.findOne({ doctorId }).sort({ isPrimary: -1, sortOrder: 1, createdAt: 1 }).select('name phone').lean(),
  ]);
  const fields = {};
  // Mission-captured variables first (gbp_link, website_url, …). The doctor's
  // profile (extracted) is authoritative and overrides them when it has a value,
  // so a value set in My Profile wins; evidence inputs only fill the gaps.
  for (const [k, v] of Object.entries(profile?.variables || {})) {
    if (v != null && String(v).trim()) fields[k] = String(v).trim();
  }
  for (const f of profile?.credentials?.extracted || []) {
    if (f?.value != null && String(f.value).trim()) fields[f.field] = String(f.value).trim();
  }
  if (!fields.doctor_name) fields.doctor_name = doctor?.displayName || doctor?.name || '';
  if (!fields.specialty && doctor?.specialization) fields.specialty = doctor.specialization;

  // Unify clinic/contact tokens with the booking data (Clinic Manager + Settings)
  // so {{clinic_name}}, {{whatsapp_number}} and {{appointment_number}} always
  // match what patients actually see. The canonical booking value wins; the
  // Practice Builder profile value is the fallback.
  const canonicalClinicName = (doctor?.clinicName && doctor.clinicName.trim()) || primaryClinic?.name || '';
  if (canonicalClinicName) fields.clinic_name = canonicalClinicName;
  const canonicalWhatsapp = doctor?.whatsappNumber || '';
  if (canonicalWhatsapp) fields.whatsapp_number = canonicalWhatsapp;
  if (!fields.appointment_number) fields.appointment_number = primaryClinic?.phone || doctor?.whatsappNumber || doctor?.phone || '';

  // The doctor's live website, so {{website_url}} / {{website_link}} resolve in
  // tasks and content. Prefer a verified custom domain, else the curago subdomain.
  if (!fields.website_url) {
    try {
      const { primaryBaseUrl } = await import('@/lib/primaryDomain');
      const base = primaryBaseUrl(doctor);
      if (base) { fields.website_url = base; fields.website_link = base; }
    } catch { /* optional */ }
  }

  // Forgive common token-name variants so a button authored as
  // {{google_drive_link}} still resolves to the doctor's {{drive_link}} value,
  // {{google_business_profile}} → {{gbp_link}}, etc. Only fills a variant when
  // the canonical value exists and the variant isn't already set.
  const ALIASES = {
    drive_link: ['google_drive', 'google_drive_link', 'drive', 'google_drive_url', 'drivelink'],
    gbp_link: ['gbp', 'google_business', 'google_business_profile', 'google_business_profile_link', 'gbp_url', 'gmb_link'],
    whatsapp_number: ['whatsapp', 'whatsapp_no', 'wa_number'],
    appointment_number: ['appointment_no', 'booking_number', 'contact_number'],
    clinic_name: ['clinic'],
  };
  for (const [canonical, alts] of Object.entries(ALIASES)) {
    if (fields[canonical] != null && String(fields[canonical]).trim()) {
      for (const a of alts) if (!(fields[a] != null && String(fields[a]).trim())) fields[a] = fields[canonical];
    }
  }

  // In-order ordinal tokens: {{treatment_one}} = the doctor's 1st approved
  // treatment, {{treatment_two}} = 2nd, … (and {{disease_one}} etc. for the
  // disease each belongs to). Deterministic and in approved order, so tasks that
  // reference treatment_one/two/three resolve consistently — never a random pick.
  try {
    const PracticeOsDiseaseCluster = (await import('@/models/practice-os/PracticeOsDiseaseCluster')).default;
    const approved = await PracticeOsDiseaseCluster.find({ doctorId, approved: true }).sort({ order: 1 }).lean();
    let flat = approved.flatMap((c) => (c.treatments || []).map((t) => ({ t: t.name, d: c.name, slug: c.slug })));
    // Fallback (mirrors getTreatmentFlat in engine.js so the card and the content
    // resolve the SAME treatment_one when there are no approved clusters yet):
    // derive from the doctor's procedures/diseases list.
    if (!flat.length) {
      flat = String(fields.procedures || fields.diseases || fields.expertise || '')
        .split(/[,\n;|]/).map((s) => s.trim()).filter(Boolean)
        .map((p) => ({ t: p, d: '', slug: '' }));
    }
    const ORD = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven', 'twelve', 'thirteen', 'fourteen', 'fifteen', 'sixteen', 'seventeen', 'eighteen', 'nineteen', 'twenty', 'twentyone', 'twentytwo', 'twentythree', 'twentyfour', 'twentyfive', 'twentysix', 'twentyseven', 'twentyeight', 'twentynine', 'thirty'];

    // Blog/page link for each treatment's disease cluster, so tasks can reference
    // {{treatment_one_blog_link}} etc. Best-effort — unpublished pages stay blank.
    let clusterLinks = {};
    try {
      const { getBlogLinkRegistry } = await import('@/lib/practice-os/blogLinks');
      const reg = await getBlogLinkRegistry(doctorId);
      for (const [clusterSlug, byType] of Object.entries(reg.clusters || {})) {
        const first = Object.values(byType || {})[0];
        if (first?.url) clusterLinks[clusterSlug] = first.url;
      }
    } catch { /* registry optional */ }

    flat.forEach((item, i) => {
      const ord = ORD[i];
      if (!ord) return;
      if (item.t) fields[`treatment_${ord}`] = item.t;
      if (item.d) fields[`disease_${ord}`] = item.d;
      const link = item.slug && clusterLinks[item.slug];
      if (link) fields[`treatment_${ord}_blog_link`] = link;
    });
  } catch { /* clusters optional — tokens simply stay unfilled */ }

  return fields;
}

// True once the profile has enough substance for the AI to write grounded,
// personalized content. A fresh account (name only, e.g. from Google) is NOT
// ready — the assistant should send the doctor to finish their profile first.
export function isProfileReadyForAi(fields = {}) {
  const has = (k) => String(fields?.[k] ?? '').trim().length > 0;
  // Specialty is the core grounding a fresh (name-only) account lacks. Once the
  // doctor completes the identity block, the AI has enough to be personalized.
  return has('specialty');
}

export async function getDoctorProfileContext(doctorId) {
  const PracticeOsAccessRequest = (await import('@/models/practice-os/PracticeOsAccessRequest')).default;
  const [doctor, profile, accessReq] = await Promise.all([
    Doctor.findById(doctorId).select('name displayName specialization').lean(),
    PracticeOsProfile.findOne({ doctorId }).select('credentials').lean(),
    PracticeOsAccessRequest.findOne({ doctorId }).select('answers').sort({ createdAt: -1 }).lean(),
  ]);

  const lines = [];
  const name = doctor?.displayName || doctor?.name;
  if (name) lines.push(`Name: ${name}`);
  if (doctor?.specialization) lines.push(`Specialization: ${doctor.specialization}`);

  // Confirmed extracted CV fields.
  for (const f of profile?.credentials?.extracted || []) {
    if (f?.confirmed && f?.value) lines.push(`${FIELD_LABELS[f.field] || f.field}: ${f.value}`);
  }

  // The doctor's own words from the early-access application (location, why this
  // matters, practice status, patient baseline, …) — useful grounding for tone
  // and priorities. Skip the agreement checkbox.
  const appLines = [];
  const ans = accessReq?.answers && typeof accessReq.answers === 'object' ? accessReq.answers : {};
  for (const [k, v] of Object.entries(ans)) {
    if (k === 'agreement' || v == null || v === true || v === false) continue;
    const val = String(v).trim();
    if (val) appLines.push(`${k.replace(/_/g, ' ')}: ${val.slice(0, 300)}`);
  }

  const cvText = profile?.credentials?.cvText || '';
  if (!lines.length && !cvText && !appLines.length) return '';

  let ctx = 'This is the doctor you are helping. Use these real details for accurate, personalized drafts. Do not state anything about them beyond what is here.\n';
  if (lines.length) ctx += lines.join('\n');
  if (appLines.length) ctx += `\n\nWhat the doctor told us about their practice and goals:\n${appLines.join('\n')}`;
  if (cvText) ctx += `\n\nCV excerpt:\n${cvText.slice(0, 3000)}`;
  return ctx;
}
