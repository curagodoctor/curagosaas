...
export async function revokePracticeOsAccess(doctorId, frameworkId) {
  ...
  await syncPracticeOsActive(doctorId);
}

/**
 * Delete a single purchase record by its id (used to clear ghost / deleted-pack
 * entitlements that aren't selectable by frameworkId in the admin UI).
 */
export async function revokePurchaseById(doctorId, purchaseId) {
  if (!purchaseId) throw new Error('revokePurchaseById requires a purchaseId');
  await PracticeOsPurchase.deleteOne({ _id: purchaseId, doctorId });
  await syncPracticeOsActive(doctorId);
}

/**
 * Remove ALL of a doctor's pack entitlements (every purchase). Progress/scores
 * are left intact. Free packs remain accessible regardless.
 */
export async function revokeAllPracticeOsAccess(doctorId) {
  await PracticeOsPurchase.deleteMany({ doctorId });
  await Doctor.updateOne({ _id: doctorId }, { $set: { practiceOsActive: false } });
}

// Keep the legacy practiceOsActive flag in step with whether any completed
// purchase remains.
async function syncPracticeOsActive(doctorId) {
