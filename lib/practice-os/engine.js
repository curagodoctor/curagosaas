...
export async function getDay(doctorId, missionId) {
  ...
  // The daily-engine prompts reference {{treatment_one}}…{{treatment_thirty}}.
  // These are already resolved IN ORDER by getDoctorProfileFields (treatment_one =
  // the doctor's 1st approved treatment, etc.) — the SAME source the control-center
  // cards use. Do NOT re-derive them here: an earlier day-rotation override made
  // the day content resolve a different treatment than the card (e.g. Day 3 →
  // flat[4]), which is exactly the card/content mismatch.
  const fields = { ...profileFields, ...linkVars };
