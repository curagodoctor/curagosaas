...
import { hasPackAccess, assertPackAccess, getOptimizationAccessState } from '@/lib/practice-os/access';
...
// Cadence: max days'-worth of tasks a doctor can finish in one calendar day.
export const DAILY_TASK_CAP = 7;

// IST calendar-day index (India has no DST). Used for calendar-paced unlocking.
const istDayIndex = (d) => Math.floor((new Date(d).getTime() + IST_OFFSET_MS) / 86400000);

// The effective "now" for calendar unlocking. For optimization packs, once access
// ends (grace or locked) we FREEZE unlocking at the access-end moment — so no new
// tasks appear after the subscription/free-access lapses; only the already-unlocked
// backlog remains (to be cleared during the grace window).
async function calendarUnlockNow(doctorId, framework) {
  if ((framework?.tier || 'optimization') === 'optimization') {
    try {
      const { phase, accessEndAt } = await getOptimizationAccessState(doctorId);
      if ((phase === 'grace' || phase === 'locked') && accessEndAt) return new Date(accessEndAt);
    } catch { /* fall through to real now */ }
  }
  return new Date();
}
// For a calendar-paced pack, how many curriculum days are unlocked: day 1 on the
// start day, +1 for each calendar day since (IST), regardless of completion.
function calendarUnlockThroughDay(startedAt, now = new Date()) {
  if (!startedAt) return 1;
  return Math.max(1, istDayIndex(now) - istDayIndex(startedAt) + 1);
}
...
// Flat list of the doctor's APPROVED disease→treatment pairs (fallback: profile
// procedures/diseases). Used to fill {{treatment_one}}/{{treatment_two}} tokens.
async function getTreatmentFlat(doctorId, profileFields = null) {
  let flat = [];
  try {
    const PracticeOsDiseaseCluster = (await import('@/models/practice-os/PracticeOsDiseaseCluster')).default;
    const approved = await PracticeOsDiseaseCluster.find({ doctorId, approved: true }).sort({ order: 1 }).lean();
    flat = approved.flatMap((c) => (c.treatments || []).map((t) => ({ t: t.name, d: c.name })));
  } catch { /* ignore */ }
  if (!flat.length && profileFields) {
    const procs = String(profileFields.procedures || profileFields.diseases || profileFields.expertise || '')
      .split(/[,\n;|]/).map((s) => s.trim()).filter(Boolean);
    flat = procs.map((p) => ({ t: p, d: '' }));
  }
  return flat;
}
// Ordinal words used by the authored tasks: {{treatment_one}} … {{treatment_thirty}}.
export const TREATMENT_ORDINALS = [
  'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten',
  'eleven', 'twelve', 'thirteen', 'fourteen', 'fifteen', 'sixteen', 'seventeen', 'eighteen', 'nineteen', 'twenty',
  'twentyone', 'twentytwo', 'twentythree', 'twentyfour', 'twentyfive', 'twentysix', 'twentyseven', 'twentyeight', 'twentynine', 'thirty',
];

// A STABLE, in-order map of the doctor's treatments to ordinal tokens:
// treatment_one = the 1st treatment in the approved list, treatment_two = 2nd, …
// (and disease_one/two/… for the disease each belongs to). This is deterministic
// and follows the doctor's approved order — never a random/rotating pair.
export function treatmentVars(flat) {
  const v = {};
  (flat || []).forEach((item, i) => {
    const ord = TREATMENT_ORDINALS[i];
    if (!ord) return;
    v[`treatment_${ord}`] = item.t;
    v[`disease_${ord}`] = item.d;
  });
  return v;
}
function fillTreatmentTokens(text, vars) {
  let s = String(text || '');
  for (const [k, val] of Object.entries(vars || {})) s = s.replace(new RegExp(`\\{\\{\\s*${k}\\s*\\}\\}`, 'g'), val || '');
  return s;
}
...
// The list of tasks the doctor can pick up now. Sequence-paced → just the single
// next task. Calendar-paced → every unlocked-but-incomplete task (the backlog),
// so pending work accumulates and can be done in any order.
async function findPendingMissions(doctorId, frameworkId, pacing, startedAt, unlockNow = new Date()) {
  const days = await Mission.find({ frameworkId, status: 'published' })
    .sort({ weekNumber: 1, dayNumber: 1, missionNumber: 1, createdAt: 1 })
    .select('missionText category weekNumber dayNumber').lean();
  const prog = await UserMissionProgress.find({ doctorId, frameworkId, status: { $in: ['completed', 'skipped'] } })
  ...
  const doneSet = new Set(prog.map((p) => String(p.missionId)));
  const incomplete = [];
  days.forEach((d, i) => { if (!doneSet.has(String(d._id))) incomplete.push({ d, i }); });
  // Fill {{treatment_one}}/{{treatment_two}} in the task titles from the doctor's
  // approved diseases (rotating per day), so the card never shows raw tokens.
  const flat = await getTreatmentFlat(doctorId, await getDoctorProfileFields(doctorId));
  const toObj = ({ d, i }) => {
    const displayDay = pacing === 'calendar' ? (d.dayNumber || i + 1) : i + 1;
    return {
      id: String(d._id),
      title: fillTreatmentTokens(d.missionText, treatmentVars(flat)),
      category: d.category,
  ...
  if (pacing === 'calendar') {
    const through = calendarUnlockThroughDay(startedAt, unlockNow);
    return incomplete.filter(({ d, i }) => (d.dayNumber || i + 1) <= through).map(toObj);
  }
  return incomplete.length ? [toObj(incomplete[0])] : [];
}
...
export async function listPacksForDoctor(doctorId) {
  // Packs this doctor already owns (purchased or enrolled) — they must keep
  // seeing these on their control center even if the pack is later HIDDEN from
  // the public catalogue.
  const [purchases, enrollments] = await Promise.all([
    PracticeOsPurchase.find({ doctorId, status: 'completed', frameworkId: { $ne: null } }).select('frameworkId').lean(),
    PracticeOsEnrollment.find({ doctorId }).select('frameworkId').lean(),
  ]);
  const ownedIds = [...new Set([...purchases, ...enrollments].map((x) => String(x.frameworkId)).filter(Boolean))];

  // Show catalogue packs (published + not hidden) PLUS any pack the doctor owns.
  const frameworks = await Framework.find({
    isActive: true,
    $or: [
      { isPublished: true, isHidden: { $ne: true } },
      { _id: { $in: ownedIds } },
    ],
  ...
  for (const fw of frameworks) {
    ...
    let progress = null, xp = 0, streak = 0, visibility = 0, nextUp = null, pending = [], started = false;
    let scheduledFor = null, scheduleWindow = '';
    if (owned) {
      let enr = await PracticeOsEnrollment.findOne({ doctorId, frameworkId: fw._id }).lean();
      // Auto-enroll the granted optimization (daily-engine) pack — active + started
      // — so its daily tasks begin unlocking without a separate "start pack" step.
      if (!enr && (fw.tier || 'optimization') === 'optimization') {
        try {
          await PracticeOsEnrollment.updateOne(
            { doctorId, frameworkId: fw._id },
            { $setOnInsert: { doctorId, frameworkId: fw._id, status: 'active', setupComplete: true, startedAt: new Date() } },
            { upsert: true },
          );
          enr = await PracticeOsEnrollment.findOne({ doctorId, frameworkId: fw._id }).lean();
        } catch { /* ignore races */ }
      }
      if (enr) {
        ...
        pending = await findPendingMissions(doctorId, fw._id, fw.pacing, enr.startedAt, await calendarUnlockNow(doctorId, fw));
    ...
    packs.push({
      ...
      nextUp,
      pending,
      pacing: fw.pacing || 'sequence',
      tier: fw.tier || 'optimization',
      scheduledFor,
      scheduleWindow,
    });
  }
...
export async function computeState(doctorId, frameworkId) {
  ...
  // Calendar-paced packs unlock one mission per calendar day from the start,
  // regardless of completion — so past-due missions accumulate and are ALL
  // available to pick in any order.
  const calendarPaced = framework.pacing === 'calendar';
  // Freeze unlocking at access-end during grace/locked (no new tasks after lapse).
  const unlockThrough = calendarPaced ? calendarUnlockThroughDay(enr.startedAt, await calendarUnlockNow(doctorId, framework)) : 0;
  const daysWithStatus = days.map((d, i) => {
    const p = progByMission.get(String(d._id));
    let status;
    if (p?.status === 'completed') status = 'completed';
    else if (p?.status === 'skipped') status = 'skipped';
    else if (calendarPaced) {
      // Available once its calendar day has arrived (authored dayNumber, so the
      // two-task days unlock together); no dependence on finishing earlier tasks.
      status = (p?.manuallyUnlocked || (d.dayNumber || i + 1) <= unlockThrough) ? 'available' : 'locked';
    }
    else if (i === currentIndex) {
      // §6 — sequence pacing: after finishing a mission, the next unlocks at the
      // next midnight (IST), so a doctor can't work ahead. Available when there's
      // no lock (first mission), the lock time has passed (catch-up when behind is
      // always fine), or an admin manually unlocked it.
      const lockedUntil = enr.nextUnlockAt ? new Date(enr.nextUnlockAt).getTime() : 0;
      status = (p?.manuallyUnlocked || now.getTime() >= lockedUntil) ? 'available' : 'locked';
    } else status = 'locked';
    return {
      _id: String(d._id),
...
export async function getDay(doctorId, missionId) {
  ...
  const profileFields = await getDoctorProfileFields(doctorId);
  // §10 — merge the central blog-link registry so tokens like
  // {{disease_cluster_gallbladder_stones_treatment_page}} resolve in prompts,
  // step copy AND button URLs (dynamic redirection to the doctor's own pages).
  let linkVars = {};
  try { linkVars = (await getBlogLinkRegistry(doctorId)).variables || {}; } catch { /* best-effort */ }
  // The daily-engine prompts reference {{treatment_one}}/{{treatment_two}} (and
  // their diseases). Fill them from the doctor's APPROVED disease clusters,
  // rotating a fresh pair per day so each day's content covers new treatments.
  const treatVars = {};
  try {
    const PracticeOsDiseaseCluster = (await import('@/models/practice-os/PracticeOsDiseaseCluster')).default;
    const approved = await PracticeOsDiseaseCluster.find({ doctorId, approved: true }).sort({ order: 1 }).lean();
    const flat = approved.flatMap((c) => (c.treatments || []).map((t) => ({ t: t.name, d: c.name })));
    if (flat.length) {
      const dn = Number(day?.dayNumber) || 1;
      const a = flat[((dn - 1) * 2) % flat.length];
      const b = flat[(((dn - 1) * 2) + 1) % flat.length];
      if (a) { treatVars.treatment_one = a.t; treatVars.disease_one = a.d; }
      if (b) { treatVars.treatment_two = b.t; treatVars.disease_two = b.d; }
    }
  } catch { /* best-effort */ }
  // Fallback (no approved clusters yet): use the doctor's stated procedures /
  // diseases so the prompt is never left with a raw {{treatment_one}} token.
  if (!treatVars.treatment_one) {
    const procs = String(profileFields.procedures || profileFields.diseases || profileFields.expertise || '')
      .split(/[,\n;|]/).map((s) => s.trim()).filter(Boolean);
    const dn = Number(day?.dayNumber) || 1;
    if (procs.length) {
      treatVars.treatment_one = procs[((dn - 1) * 2) % procs.length];
      treatVars.treatment_two = procs[(((dn - 1) * 2) + 1) % procs.length];
    }
  }
  const fields = { ...profileFields, ...linkVars, ...treatVars };
  // The treatment this day is ABOUT (the first {{treatment_*}} token it uses,
  // resolved). The day interface uses this to detect and refresh stale cached
  // content — content drafted for a different treatment (after a map change).
  const rawForTopic = `${day.missionText || ''} ${(modules[0]?.aiPrompt) || ''}`;
  const topicKey = (rawForTopic.match(/\{\{\s*(treatment_[a-z]+)\s*\}\}/i) || [])[1];
  const primaryTreatment = topicKey ? (fields[topicKey] || '') : '';
  ...
  // Fill the mission's own copy (title/purpose/description) too, so the day
  // header never shows raw {{treatment_one}} tokens.
  day.missionText = fillPlaceholders(day.missionText, fields);
  day.objective = fillPlaceholders(day.objective, fields);
  day.purpose = fillPlaceholders(day.purpose, fields);
  day.briefDescription = fillPlaceholders(day.briefDescription, fields);
  // Fill the dynamic primary button's link/label per-doctor (e.g. {{gbp_link}}).
  if (day.primaryAction) {
    day.primaryAction = {
      type: day.primaryAction.type || 'blog',
      label: fillPlaceholders(day.primaryAction.label, fields),
      url: fillPlaceholders(day.primaryAction.url, fields),
    };
  }
  ...
  return { day, modules, progress, enrollment: enr, packMode: fw?.mode || 'mission', primaryTreatment };
}
...
export async function completeDay(doctorId, missionId, { record, actualMinutes, nextCommitment, reflection, kpis } = {}) {
  ...
  const fwTier = (await Framework.findById(frameworkId).select('tier').lean())?.tier || 'optimization';
  const enr = await getOrCreateEnrollment(doctorId, frameworkId);
  const now = new Date();
  ...
  // View-only once the grace window is over: access ended >7 days ago, so tasks
  // can be seen but not finished until the doctor resubscribes.
  if (!wasCompleted && fwTier === 'optimization') {
    const { phase } = await getOptimizationAccessState(doctorId);
    if (phase === 'locked') {
      const e = new Error('AccessEnded');
      e.code = 'AccessEnded';
      throw e;
    }
  }

  // Cadence cap: a doctor may finish at most DAILY_TASK_CAP (7) days' worth of
  // tasks per calendar day (IST). Prevents burning a 28-day backlog in one sitting;
  // re-finishing an already-complete task doesn't count.
  if (!wasCompleted) {
    const todayStartUtc = istDayIndex(now) * 86400000 - IST_OFFSET_MS;
    const doneToday = await UserMissionProgress.countDocuments({
      doctorId, status: 'completed', completedAt: { $gte: new Date(todayStartUtc) },
    });
    if (doneToday >= DAILY_TASK_CAP) {
      const e = new Error('DailyCapReached');
      e.code = 'DailyCapReached';
      e.cap = DAILY_TASK_CAP;
      throw e;
    }
  }
  ...
  // Advance: monotonic days-completed. §6 — the next mission unlocks at the next
  // midnight reset, so a doctor is done for the day and can't work ahead.
  if (!wasCompleted) {
    ...
    enr.nextUnlockAt = nextMidnightReset(now);
    enr.aheadUsed = false;
...
export async function skipDay(doctorId, missionId) {
  ...
  enr.currentDayNumber = (day.missionNumber || enr.currentDayNumber) + 1;
  enr.nextUnlockAt = nextMidnightReset(now);   // §6 — skipping still advances one/day
  enr.lastActiveAt = now;
  await enr.save();
