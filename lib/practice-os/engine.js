...
// The treatment/disease vars for a given day — rotates a fresh pair per day.
function treatmentVarsForDay(flat, dayNumber) {
  const v = {};
  if (!flat || !flat.length) return v;
  const dn = Number(dayNumber) || 1;
  const a = flat[((dn - 1) * 2) % flat.length];
  const b = flat[(((dn - 1) * 2) + 1) % flat.length];
  if (a) { v.treatment_one = a.t; v.disease_one = a.d; }
  if (b) { v.treatment_two = b.t; v.disease_two = b.d; }
  return v;
}
...
async function findPendingMissions(doctorId, frameworkId, pacing, startedAt) {
  ...
  const toObj = ({ d, i }) => {
    ...
    return {
      id: String(d._id),
      title: fillTreatmentTokens(d.missionText, treatmentVarsForDay(flat, d.dayNumber || i + 1)),
      category: d.category,
