...
export async function generateAiImage(doctorId, topic, { kind = 'post' } = {}) {
  if (!process.env.OPENAI_API_KEY) return null;
  try {
    const { put } = await import('@vercel/blob');
    const OpenAI = (await import('openai')).default;
    const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
    const prompt = `A clean, professional, editorial healthcare illustration for a patient-education piece about "${topic}". Calm, trustworthy clinical aesthetic, soft natural lighting, muted greens and warm neutrals. No text, no words, no letters, no logos, no watermarks. Not graphic or gory.`;
