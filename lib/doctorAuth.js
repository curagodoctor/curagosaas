...
export async function getCurrentDoctor(request) {
  try {
    ...
    if (!token || token === 'null' || token === 'undefined' || !token.startsWith('eyJ')) {
      // Fallback: check for SEO user token (allows SEO users to access page editor APIs)
      let seoToken = null;
      try {
        const cookieHeader = request?.headers?.get?.('cookie');
        if (cookieHeader) {
          const parsedCookies = cookieHeader.split(';').reduce((acc, c) => {
            const t = c.trim(); const eq = t.indexOf('=');
            if (eq > 0) acc[t.substring(0, eq)] = t.substring(eq + 1);
            return acc;
          }, {});
          seoToken = parsedCookies['seo_token'];
        }
        if (!seoToken) {
          const cookieStore = await cookies();
          seoToken = cookieStore.get('seo_token')?.value;
        }
      } catch (e) { /* ignore */ }

      if (seoToken && seoToken.startsWith('eyJ')) {
        const seoDecoded = verifyDoctorToken(seoToken);
        if (seoDecoded?.role === 'seo_user' && seoDecoded?.doctorId) {
          await connectDB();
          const doctor = await Doctor.findById(seoDecoded.doctorId).select('-password');
          if (doctor && doctor.isActive) return doctor;
        }
      }

      // Fallback: check for clinic manager token (allows clinic managers to access contacts/workflows APIs)
      let cmToken = null;
      try {
        const cookieHeader = request?.headers?.get?.('cookie');
        if (cookieHeader) {
          ...
          cmToken = parsedCookies['clinic_manager_token'];
        }
        if (!cmToken) {
          const cookieStore = await cookies();
          cmToken = cookieStore.get('clinic_manager_token')?.value;
        }
      } catch (e) { /* ignore */ }

      if (cmToken && cmToken.startsWith('eyJ')) {
        const cmDecoded = verifyDoctorToken(cmToken);
        if (cmDecoded?.role === 'clinic_manager' && cmDecoded?.doctorId) {
          await connectDB();
          const doctor = await Doctor.findById(cmDecoded.doctorId).select('-password');
          if (doctor && doctor.isActive) return doctor;
        }
      }
