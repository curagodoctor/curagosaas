...
- faqs: { title, subtitle, faqs: [{ question, answer }] }  ← each FAQ item MUST contain ONLY "question" and "answer". Do NOT add a per-question subtitle or any other field.
...
Areas of expertise: ${doctorInfo.expertise || ''}
Procedures / treatments: ${doctorInfo.procedures || ''}
Services: ${formData.services || 'General medical services'}
Clinic Name: ${formData.clinicName || doctorInfo.clinicName || doctorInfo.displayName + "'s Clinic"}
Clinic Address: ${formData.location || doctorInfo.clinicAddress || ''}
City: ${doctorInfo.city || ''}${doctorInfo.state ? ', ' + doctorInfo.state : ''}
Consultation timings: ${doctorInfo.timings || ''}
Consultation fee: ${doctorInfo.consultationFee || ''}
Phone / appointment number: ${formData.phone || doctorInfo.phone || ''}
...
Use the real clinic name, address, city, timings and contact details above in the location_map and footer sections — do not invent placeholder contact details. Generate a complete JSON array of website sections.`;
