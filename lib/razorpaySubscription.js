...
// Create a monthly subscription plan (₹500/month)
...
export async function getOrCreatePlan() {
  ...
  try {
    ...
    const existing = plans.items?.find(
      p => p.item?.amount === 50000 && p.period === 'monthly' && p.interval === 1
    );
    if (existing) {
  ...
  const plan = await razorpayRequest('/plans', 'POST', {
    ...
    interval: 1,
    item: {
      name: 'CuraGo Website Builder — Monthly',
      amount: 50000, // ₹500 in paise
      currency: 'INR',
      description: 'Monthly subscription for the CuraGo Website Builder premium features',
    },
  });
