...
export function evaluateFit(a) {
  ...
  if (H.length >= 1) {
    ...
    // Nothing qualifying was answered — never call that a strong fit; route to review.
    result = 'maybe';
    reason = 'Not enough answered to confirm a strong fit.';
  } else {
    result = 'strong_fit';
    reason = `You show strong signals: ${P.slice(0, 3).join('; ')}.`;
  }
