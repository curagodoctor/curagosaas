...
export const initializeDefaultModes = async (doctorId = null) => {
  ...
  if (!doctorId) return; // Don't initialize without doctorId
  ...
  if (count === 0) {
    ...
    for (const modeData of defaultModes) {
      ...
      const mode = await ConsultationMode.findOneAndUpdate(
        { doctorId, name: modeData.name },
        { $setOnInsert: { doctorId, ...modeData } },
        { upsert: true, new: true }
      );

      // Check if weekly schedules exist for this mode
      const scheduleCount = await WeeklySchedule.countDocuments({ doctorId, modeId: mode._id });

      if (scheduleCount === 0) {
        for (let dayOfWeek = 0; dayOfWeek <= 6; dayOfWeek++) {
          await WeeklySchedule.findOneAndUpdate(
            { doctorId, modeId: mode._id, dayOfWeek },
            { $setOnInsert: { doctorId, modeId: mode._id, dayOfWeek, isEnabled: false, enabledSlots: [] } },
            { upsert: true }
          );
        }
      }
  ...
  if (count === 0) {
    ...
    for (const time of defaultTimes) {
      ...
      await TimeSlot.findOneAndUpdate(
        { doctorId, time },
        { $setOnInsert: { doctorId, time, label, isActive: true } },
        { upsert: true, new: true }
      );
    }
...
export const getAllBookings = async (doctorId = null) => {
  ...
  // Include bookings with matching doctorId OR without doctorId (fallback for bookings created before multi-tenant)
  const query = doctorId
    ? { $or: [{ doctorId }, { doctorId: { $exists: false } }, { doctorId: null }] }
    : {};
...
export const cancelBooking = async (date, time, doctorId = null) => {
  ...
  if (doctorId) booking.cancelledBy = doctorId;
  await booking.save();
