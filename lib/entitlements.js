...
/**
 * The premium features are unlocked when EITHER the website-builder subscription
 * is active OR the doctor is still working through a paid Practice Builder pack.
 * Buying Practice Builder includes the Website Builder premium tier FOR THE
 * DURATION of the pack — once they complete all its missions, this reverts and
 * they're asked to subscribe. (The reverse is never true: a website subscription
 * doesn't grant Practice Builder pack access.) Subscription is checked first so
 * active subscribers incur no extra query.
 */
export async function hasActiveEntitlement(doctorId) {
  if (await Subscription.isActive(doctorId)) return true;
  return hasActivePracticeBuilder(doctorId);
}
...
export async function getEntitlements(doctorId) {
  ...
  const subActive = await Subscription.isActive(doctorId);
  // Included while a paid Practice Builder pack is still in progress; ends when
  // the pack is completed.
  const viaPracticeBuilder = subActive ? false : await hasActivePracticeBuilder(doctorId);
  const isActive = subActive || viaPracticeBuilder;
  ...
  return {
    ...
    status: sub.status,
    isActive,
    viaPracticeBuilder,   // true = unlocked because they own Practice Builder
    daysRemaining,
...
export async function hasFeature(doctorId, feature) {
  ...
  return hasActiveEntitlement(doctorId);
}
  ...
  return NextResponse.json(
    {
      ...
      upgrade: {
        required: true,
        message: 'Upgrade your plan or redeem a promo code to unlock Contacts, Workflows, Templates and Messaging.',
        url: UPGRADE_URL,
      },
    },
    { status: 403 }
