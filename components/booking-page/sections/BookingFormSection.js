...
export default function BookingFormSection({
  ...
  doctor, // The tenant doctor (for their own WhatsApp number + name)
}) {
  // WhatsApp fallback contact = THIS doctor's own number (never a hardcoded one).
  // Empty when the doctor hasn't set a number, so the button is hidden rather
  // than pointing patients at the wrong person.
  const waNumber = normalizeWhatsAppNumber(doctor?.whatsappNumber || doctor?.phone || '');
  const waDoctorName = doctor?.displayName || doctor?.name || customTitle || 'the doctor';
  ...
  return (
    <section id={sectionId || "booking_form"} className="container mx-auto px-4 md:px-6 py-4 md:py-12 lg:py-16">
      <div className="max-w-4xl lg:max-w-5xl mx-auto">
                ...
                <div>
                  {isToday(selectedDate) ? (
                    ...
                    <div className="bg-yellow-50 border-2 border-yellow-200 rounded-2xl p-6 md:p-8">
                      ...
                      {/* WhatsApp CTA Button — the doctor's OWN number */}
                      {waNumber && (
                        <a
                          href={`https://wa.me/${waNumber}?text=${encodeURIComponent(`Hi! I want to book a consultation with ${waDoctorName} for today`)}`}
                          target="_blank"
        ...
        {/* Help — the doctor's OWN WhatsApp number (hidden if not set) */}
        {waNumber && (
          <div className="mt-4 md:mt-8 text-center text-primary-600">
            <p className="text-xs md:text-sm">
              Need help?{" "}
              <a
                href={`https://wa.me/${waNumber}?text=${encodeURIComponent(`Hi, I need help with my booking with ${waDoctorName}.`)}`}
                target="_blank"
                rel="noopener noreferrer"
