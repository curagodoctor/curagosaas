...
export default function ChatbotConfig({ config = {}, onChange }) {
  ...
  const resetToDefaults = () => {
    if (confirm("This will clear all FAQ content so you can start fresh. Continue?")) {
      onChange({
        enabled: true,
            ...
            <div>
              ...
              <div className="flex gap-2 max-w-xs">
                ...
                <input
                  ...
                  value={chatConfig.primaryColor}
                  onChange={(e) => handleChange("primaryColor", e.target.value)}
                  className="w-28 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                />
              </div>
            </div>
