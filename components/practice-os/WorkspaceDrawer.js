...
export default function WorkspaceDrawer() {
  ...
  // The single floating FAB opens Notes via this event (no own launcher button).
  useEffect(() => {
    const openNotes = () => setOpen(true);
    window.addEventListener('pos:open-notes', openNotes);
    return () => window.removeEventListener('pos:open-notes', openNotes);
  }, []);
  ...
  return (
    <>
      {/* Launcher buttons removed — the single floating FAB opens this drawer. */}
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b shrink-0" style={{ borderColor: 'var(--rule)' }}>
              ...
              <div className="flex items-center gap-3">
                <Link href="/app/control-center/workspace" className="pos-link text-[12px]" style={{ color: 'var(--muted)' }}>Open full</Link>
