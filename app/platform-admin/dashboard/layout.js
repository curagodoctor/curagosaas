...
export default function PlatformAdminDashboardLayout({ children }) {
  ...
  const navigation = [
    ...
    {
      name: 'Rep Managers',
      href: '/dashboard/reputation-managers',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const isActive = (href) => {
    if (href === '/dashboard') {
      return pathname === '/platform-admin/dashboard' || pathname === '/dashboard';
    }
    // Check both with and without /platform-admin prefix for compatibility
    const normalizedPathname = pathname.replace('/platform-admin', '');
