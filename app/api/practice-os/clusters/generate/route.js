...
export async function POST(request) {
  try {
    ...
    const gen = await structureContent({
      instruction: `You are mapping a doctor's practice for content generation. Based ONLY on the specialty "${specialty}", generate EXACTLY 10 distinct diseases/conditions this specialty commonly treats — ordered most-common first. For EACH disease, list its own SPECIFIC treatments — minimum 1, maximum 2 — that are genuinely used for THAT disease (never generic, never repeated across diseases unless truly the same treatment). Return JSON: {"diseases": [{"name": string, "treatments": string[] (1-2 items)}]} with exactly 10 diseases. Use correct medical terminology paired with plain patient-facing names. NMC-compliant — factual, no superlatives, no outcome claims.`,
