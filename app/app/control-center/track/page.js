...
function TrackView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 402) { router.push(`/app/control-center/unlock?pack=${packId}`); return; }
      if (res.status === 404) { router.replace('/app/control-center'); return; }
      const data = await res.json();
      if (!data.success) return;
      if (!data.enrollment.setupComplete) { router.push(`/app/control-center/setup?pack=${packId}`); return; }
      setState(data);
    } finally {
      setLoading(false);
    }
...
function MissionIntro({ day, full, modules = [], progress, loaded, totalDays, withPack, taskMode = false }) {
  ...
  return (
        <div className="pos-card p-4 md:p-5">
          ...
          <div className="mt-4">
            <Link href={withPack(`/app/control-center/focus/${d._id}?start=1`)} className="pos-action pos-focusable inline-flex items-center gap-2" style={{ background: 'var(--orange)' }}>
...
function AllComplete({ daysCompleted, withPack }) {
  return (
    <div className="pos-card p-8">
      ...
      <Link href={withPack('/app/control-center/record')} className="pos-action inline-block mt-6">See your record</Link>
    </div>
  );
}
