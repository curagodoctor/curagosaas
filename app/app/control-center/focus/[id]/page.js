...
function FocusSession() {
  ...
  useEffect(() => {
    if (!packId) { router.replace('/app/control-center'); return; }
    (async () => {
  ...
  const finishModule = useCallback(async () => {
    ...
    if (reviewMode) {
      if (isLast) {
        router.push(`/app/control-center/track?pack=${packId}`);
      } else {
        setIndex((i) => i + 1);
  ...
  const startNextNow = () => {
    ...
    router.push(`/app/control-center/focus/${tomorrow._id}?pack=${packId}`);
  };
    ...
    return (
      <div className="min-h-screen px-3 sm:px-4 lg:px-6 py-6">
        <div className="max-w-7xl mx-auto">
          <Link href={`/app/control-center/track?pack=${packId}`} className="pos-link text-sm">← Back to pack</Link>
          ...
          {tomorrow ? (
            <>
              ...
              <div className="text-center">
                <button onClick={() => router.push(`/app/control-center/track?pack=${packId}`)} className="pos-link text-sm">Back to pack</button>
              </div>
            </>
          ) : (
            <div className="pos-card p-5 mb-2 text-center">
              <p className="text-[var(--ink)] font-medium">That was the last {noun} in this pack. 🎉</p>
              <button onClick={() => router.push(`/app/control-center/track?pack=${packId}`)} className="pos-action mt-4">See your record →</button>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="min-h-screen px-3 sm:px-4 lg:px-6 py-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
          <Link href={`/app/control-center/track?pack=${packId}`} className="pos-link text-sm shrink-0">← Exit {taskMode ? 'task' : 'mission'}</Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_440px] gap-6 lg:gap-8">
          ...
          <div className="min-w-0">
            ...
            {mod.buttons?.length > 0 && (() => {
              // Resolve {{variables}} in each button's URL/label. Skip a button
              // whose URL is a placeholder the doctor hasn't filled yet (empty),
              // so no dead links show. Ensure a scheme so it opens externally.
              const resolved = mod.buttons
                .map((b) => {
                  let url = fillVars(b.url, sessionVars).trim();
                  if (url && !/^https?:\/\//i.test(url) && !/^(mailto:|tel:)/i.test(url)) url = `https://${url}`;
                  return { url, label: fillVars(b.label, sessionVars) };
                })
                .filter((b) => b.url);
            ...
            {mod.inputs?.length > 0 && (
              <div className="pos-card p-5 mb-4" style={{ borderColor: 'var(--green)', background: 'var(--green-soft)' }}>
                ...
                <p className="text-[12.5px] text-[var(--muted)] mb-3">Keep proof of what you built — it becomes part of your record and your Day 28 completion.</p>
            ...
            <div className="flex items-center gap-5 mt-2 flex-wrap">
              ...
              <Link href={`/app/control-center/track?pack=${packId}`} className="pos-link text-sm">Leave for now</Link>
            </div>
          </div>
