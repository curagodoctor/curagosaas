...
function UnlockPack() {
  ...
  const done = useCallback(() => router.replace(`/app/control-center/track?pack=${packId}`), [router, packId]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!data.success) { router.replace('/app/control-center'); return; }
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-12">
      <Link href="/app/control-center" className="flex items-center gap-2 mb-10">
      ...
      <Link href="/app/control-center" className="pos-link text-sm inline-block mt-6">← Back to all packs</Link>
    </div>
  );
}
