...
export function Spine({ days, totalDays, withPack, mode = 'mission' }) {
  ...
  const focusHref = (d) => (withPack ? withPack(`/app/control-center/focus/${d._id}`) : `/app/control-center/focus/${d._id}`);
  const recordHref = (d) => {
    if (d.status === 'completed' || d.status === 'skipped') return `${focusHref(d)}&review=1`;
    if (d.status === 'available') return focusHref(d);
    return withPack ? `${withPack('/app/control-center/journey')}&view=record` : `/app/control-center/journey?view=record`;
  };
    ...
    <div className="space-y-4 sticky top-6">
      ...
      {summary && (
        <div className="pos-card p-5">
          ...
          <Link href="/app/control-center/profile" className="pos-link text-[12px] inline-block mt-2">Edit profile →</Link>
        </div>
      )}
